...
// впрочем, утверждение выше, вероятно не верное, и вероятно компиляторы проектируют так, чтобы размер указателя
// соответствовал размеру самого большого нативного типа в системе http://ru.cppreference.com/w/cpp/language/types
// http://cpp-code.ru/blog2/sites/default/files/download/fundamental_types_cppreference.com.7z [mirrored link]
...
// в списке инициализации выгоднее использовать запрет исключений оператора new (std::nothrow), чтобы иметь возможность
...
// по подобным рассуждениям будут работать и арифетические операторы ниже. Их существенным отличием является то что они
...
{   //это нетипичная перегрузка оператора ^, для возведения в степень,
	...
	try
	{
		for (; i < n; i++)
		{
			...
			if (!heap_symbolic_val && !overflow_multiplication(ret, n))
			{
				ret *= stack_val;
			}
...
SmartInteger SmartInteger::operator/(long long n)
{
	if (!heap_symbolic_val && !overflow_division(stack_val, n))
	{
		return SmartInteger(stack_val / n);
	...
	{
		// в том единственном случае, когда при делении
		// целочисленных типов возможно переполнение,
		// мы справимся бустом
		cpp_int tmp(stack_val);
	...
	{
		return SmartInteger(static_cast<cpp_int>(*heap_symbolic_val / n));
	}
}
